#for testing the playlists page
...
def playPlaylist():
    ...
    #playPl = MusicPlayer(current_playlist_id)
    #playPl.play()
    messagebox.showinfo("yay", "Playlist is playing")

#expected output in the tkinter gui, to have a list of playlists, that you can create a playlist (with the inital song just set to 1)
#see incrementing playlist names before renamed- which when renamed appear in the title and the playlist list. 
#deleting a playlist removes from the list
#tested to make sure that then the one uderneath in the listbox no longer says unknown and also updates to be the correct playlist name when selected
#when the upload cover button is pressed it opens a file explorer when the image is selected, it will work if you go off and back into it and then
#it will stay if you close and reopen the window ect.
#expected to see that since there are no songs downloaded in this test environment it will say no songs in this playlist, as the test song id actually is an empty cell in the database 
#play playlist button is just there while waiting for that functionality from another part, it is determined to work if it sends the message when the button is clicked, that's what that is testing
